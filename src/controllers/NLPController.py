...
class NLPController(BaseController):
    ...
    def index_into_vector_db(self, project: Project, chunks: List[DataChunk],chunks_ids:list[int],
        ...
        vectors = [
            self.embedding_client.embed_text(text=text, document_type=DocumentTypesEnums.DOCUMENT.value)
            for text in texts 
        ]

        # step3: create collection if not exists
        _ = self.vectordb_client.create_collection(
            collection_name=collection_name, embedding_size=self.embedding_client.embedding_size,
            do_reset=do_reset
        )
    ...
    def sreach_vector_db_collection(self, project: Project, text:str, limit:int=4):
        ...
        #step 2: get text embedding vecore 
        vector = self.embedding_client.embed_text(text=text, document_type=DocumentTypesEnums.QUERY.value)
    ...
    def answer_rag_question(self, project: Project, query: str, limit: int=5):
        ...
        # We combine the system instructions with the retrieved context
        ...
        return answer, full_instructions, chat_history